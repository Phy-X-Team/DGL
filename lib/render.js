var DRAW_FUNC, LINE_FUNC, ShaderSource, VertexBuffer, a_Color_location, a_Vertex_location, buf_locationVertex, colorizeArray, compileShader, data_transform, drawCircle, drawFCircle, drawFPolygon, drawFRect, drawPolygon, drawRect, drawTriangleFan, drawTriangles, frag, gLProgram, round, shape, u_PointSize_location, u_Transform_location, vert, vertexCount;
...
colorizeArray = function(array = new Float32Array, color) {
...
drawTriangles = function(triangleCount = 1, size = 1, startAngle = 0, totalAngle = 2 * Math.PI, z = 0) {
...
drawPolygon = function(count, size, startAngle, totalAngle) {
  return drawTriangles(count - 2, size, startAngle, totalAngle);
};

drawRect = function(size) {
  return drawPolygon(4, size, Math.PI / 4);
};

drawCircle = function(size, points = 100) {
  return drawPolygon(points, size);
};
...
render = function(objects) {
  var color, data_color, data_vertex, label, parameters, pointSize, ref, results, rotateX, rotateY, rotateZ, scaleX, scaleY, scaleZ, translateX, translateY, translateZ;
  //context.draw()
  if (!gLProgram) {
    return;
  }
